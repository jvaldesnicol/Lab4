...
void processTiles_weightStatinary(int numNeurons,
   ...
   int currentTileSize = inputTileSize;
       ...
       // Create buffers for remaining required data
   ...
   int numTiles = inputSize / inputTileSize; // Ensure this division is an integer: 28
   int totalWeights = inputSize * numNeurons; // 7840
   int weightsPerTile = numNeurons*inputTileSize; // Assuming an even distribution of neurons per tile: 280

   for (int tileIndex = 0; tileIndex < numTiles; ++tileIndex) { // 28 loops
      
       int weightsStartIndex = tileIndex * inputTileSize; // i*28
       ...
       std::vector<float> outputSlice(numNeurons, 0.0f); 
       ...
       clFinish(queue); 
        ...
        err = clEnqueueReadBuffer(queue, outputBuffer, CL_FALSE, 0, outputNeuronsTileSize * sizeof(float), outputSlice.data(), 0, NULL, NULL);
        clFinish(queue); 
        
        for(int i=0;i<numNeurons;i++){
            outputs[i] += outputSlice[i];//outputBuffer[i] + biases[i];
        }  
    }   

    for(int i=0;i<numNeurons;i++){
       outputs[i] += biases[i];//outputBuffer[i] + biases[i];
   }  
...
void run() {
   ...
   // Connect the layers and compute the output using FPGA functions
